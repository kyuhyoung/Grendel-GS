...
#        WAIT_GPUS=6 nohup bash run_letitcrash_b200.sh > /dev/null 2>&1 &   (대기 모드: 빈 카드 ≥6 되면 자동 시작)
#   env: MAX_GPUS(기본 6) MASTER_PORT(기본 29601) OUT(기본 ./output/lic_b200) SRC(데이터 경로)
#        WAIT_GPUS(대기 모드 문턱; --wait 만 주면 2) WAIT_INTERVAL(기본 600s) MAX_WAIT_H(기본 48h → 초과 시 rc=3)
#
set -uo pipefail
...
DRY_RUN=false; WAIT_MODE=false
for a in "$@"; do case "$a" in --dry-run) DRY_RUN=true;; --wait) WAIT_MODE=true;; esac; done
[ -n "${WAIT_GPUS:-}" ] && WAIT_MODE=true
WAIT_GPUS="${WAIT_GPUS:-2}"; WAIT_INTERVAL="${WAIT_INTERVAL:-600}"; MAX_WAIT_H="${MAX_WAIT_H:-48}"
...
if ${WAIT_MODE}; then
    ...
    while [ "${N_FREE}" -lt "${WAIT_GPUS}" ]; do
        ...
        ${DRY_RUN} && { echo "(dry-run — 대기 루프 확인 후 종료)"; exit 0; }
        sleep "${WAIT_INTERVAL}"
        probe_free_gpus
    done
    echo "[wait] $(date '+%m/%d %H:%M:%S') 빈 카드 ${N_FREE}장 [${FREE_IDS[*]}] ≥ ${WAIT_GPUS} — 시작 ($(( ($(date +%s) - WAIT_T0)/60 ))분 대기)"
elif [ "${N_FREE}" -lt 2 ]; then
    echo "!!! 빈 GPU ${N_FREE}장 (<2) — 시작 거부. 타 사용자와 공유하는 카드는 OOM 신호를 오염시키므로 쓰지 않는다."
    echo "    (자리 날 때까지 기다리려면 --wait 또는 WAIT_GPUS=N)"
    exit 2
fi
...
RUN_NAME="lic_b200_$(date +%m%d)"                   # 대기 모드면 실제 시작일 기준

# ---------- MASTER_PORT: 비어 있는지 확인 후 고정 ----------
if ss -ltn 2>/dev/null | grep -qE ":${MASTER_PORT} "; then
    echo "!!! MASTER_PORT ${MASTER_PORT} 사용 중 — MASTER_PORT=<다른 값> 으로 재실행"; exit 4
fi
