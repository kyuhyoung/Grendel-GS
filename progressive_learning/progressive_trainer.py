...
class ProgressiveTrainer:
    ...
    def __init__(
        ...
        self.camera_centers = {}  # S_c for each camera
            ...
            with open(cameras_file, 'r') as f:
                for line in f:
                    ...
                    self.cameras[cam_id] = {
                        ...
                        'width': int(parts[2]),
                        'height': int(parts[3]),
                        'params': params_list,  # Keep original list format for saving
                        'params_dict': params_dict  # Add dict format for visualizer
                    }
    ...
    def compute_camera_footprints(self):
        ...
        for img_id, img_data in self.images.items():
            ...
            if self.dtm_module:
                ...
                try:
                    ...
                    if len(corners_3d) >= 3:
                        ...
                        # Filter points within footprint
                        ...
                        W_c = []
                        for point_xy in self.W:
                            if np.linalg.norm(point_xy - S_c) <= radius:
                                W_c.append(point_xy)
                        W_c = np.array(W_c) if W_c else np.array([])
            ...
            self.camera_centers[img_id] = S_c
            
            # Handle W_c properly (could be list or numpy array)
            if isinstance(W_c, np.ndarray):
                self.points_in_footprint[img_id] = W_c if len(W_c) > 0 else np.array([])
            else:
                self.points_in_footprint[img_id] = np.array(W_c) if W_c else np.array([])
    ...
    def select_initial_cameras(self) -> List[int]:
        ...
        U = np.median(positions, axis=0)
        M = U[:2]  # (x,y) coordinate of median
        
        if self.debug:
            print(f"Median camera position: {U}")
            print(f"Median (x,y): {M}")
        
        # Step 6-7: Find m cameras closest to median
        distances = []
        for img_id, S_c in self.camera_centers.items():
            dist = np.linalg.norm(S_c - M)
    ...
    def create_initial_dataset(self, selected_cameras: List[int]) -> Dict:
        ...
        if self.debug:
            try:
                ...
                self.visualizer = COLMAPVisualizer()
                self.visualizer.set_external_data(self.cameras, self.images, self.points3D)

                # Create DTM for visualization
                print("Creating DTM for visualization...")
                self.visualizer.create_dtm(resolution=2.0)  # Coarser resolution for performance
                print("DTM created successfully")
            except Exception as e:
                print(f"Warning: Could not create visualizer/DTM: {e}")
                self.visualizer = None

            self._visualize_original_vs_subset(B, selected_cameras)

        print(f"Initial dataset: {len(B['images'])} images, {len(B['points3D'])} points")
    ...
    def _visualize_original_vs_subset(self, subset_dataset: Dict, selected_cameras: List[int]):
    ...
    def train_grendel_gs(self, dataset: Dict, iteration_name: str = "initial") -> None:
        """
    ...
    def find_next_closest_camera(self) -> Optional[int]:
        ...
        positions = np.array([img['position'] for img in self.images.values()])
        M = np.median(positions, axis=0)[:2]
        
        best_id = None
        best_dist = float('inf')
        
        for img_id in available:
            S_c = self.camera_centers[img_id]
            dist = np.linalg.norm(S_c - M)
            if dist < best_dist:
                best_dist = dist
                best_id = img_id
    ...
    def expand_until_memory_limit(self):
        ...
        while self.get_gpu_memory_usage() < self.gpu_memory_threshold:
            ...
            W_j = self.points_in_footprint[j]
            R_j = []
            for point_xy in W_j:
                if tuple(point_xy) not in self.H:
                    R_j.append(point_xy)
                    self.H.add(tuple(point_xy))
    ...
    def run(self):
        ...
        # Step 4: Compute camera footprints
        self.compute_camera_footprints()
        # Steps 5-7: Select initial cameras
        initial_cameras = self.select_initial_cameras()
        self.P = set(initial_cameras)
        
        # Step 8: Create initial dataset
        B = self.create_initial_dataset(initial_cameras)
        #print(f'aaa');  exit(1) 
        
        # Step 9: Train initial Gaussian set
        self.train_grendel_gs(B, "initial")
        
        # Step 10: Expand until memory limit
        self.expand_until_memory_limit()
        
        # Steps 11-12: Process remaining cameras with swapping
        self.swap_gaussians_for_remaining()
        
        print("="*60)
        print("Progressive Training Complete!")
        print(f"Processed {len(self.P)} cameras")
