...
def main():
    ...
    parser.add_argument('--initial_cameras', type=int, default=2, help='Number of initial cameras')
    ...
    parser.add_argument('--iterations_per_window', type=int, default=60, help='Iterations per window')
    ...
    parser.add_argument('--densify_memory_limit_percentage', type=float, default=0.90, help='GPU memory limit percentage for densification (default: 0.90)')
    ...
    parser.add_argument('--removal_strategy', type=str, default='farthest', choices=['fifo', 'farthest'], help='Camera removal strategy: fifo (oldest first) or farthest (farthest from new camera)')
    ...
    # Balanced smooth trajectory strategy parameters (5-force)
    ...
    # Load COLMAP data
    trainer._load_colmap_data()

    # Run the progressive training pipeline with dynamic window
    try:
        iterations_per_window = getattr(args, 'iterations_per_window', 60)
        trainer.run(iterations_per_window=iterations_per_window)
        print("\n" + "="*60)
