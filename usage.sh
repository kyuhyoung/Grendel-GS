...
if pip install submodules/diff-gaussian-rasterization; then
    ...
    torchrun --standalone --nnodes=1 --nproc-per-node=$GPU_COUNT train.py --bsz 1 -s $DIR_DATA --model_path ./output/$SCENE --preload_dataset_to_gpu_threshold 2 --densification_interval 200 --backend gsplat
fi
