...
if not hasattr(torch.distributed, 'all_gather_into_tensor') and hasattr(torch.distributed, '_all_gather_base'):
...
def distributed_preprocess3dgs_and_all2all_final(
    ...
    for i, (viewpoint_camera, strategy) in enumerate(
        ...
        if means3D.shape[0] == 0:
            ...
            conic_opacity = torch.zeros((0, 4), device="cuda", requires_grad=True)
            radii = torch.zeros((0,), device="cuda", dtype=torch.float32)
            depths = torch.zeros((0,), device="cuda", dtype=torch.float32)
        else:
            means2D, rgb, conic_opacity, radii, depths = rasterizer.preprocess_gaussians(
                means3D=means3D,
                scales=scales,
...
def gsplat_distributed_preprocess3dgs_and_all2all_final(
    ...
    if utils.DEFAULT_GROUP.size() == 1:
        batched_screenspace_pkg = {
            ...
            "batched_rgb_redistributed": batched_colors,
            "batched_conic_opacity_redistributed": batched_conics,  # Inverse of the projected covariances. Flattened upper triangle with (B, N, 3).
    ...
    batched_screenspace_pkg = {
        ...
        "batched_cuda_args": batched_cuda_args,
        "batched_means2D_redistributed": batched_means2D_redistributed,
        "batched_rgb_redistributed": batched_colors_redistributed,
        "batched_conic_opacity_redistributed": batched_conics_redistributed,
        "batched_radii_redistributed": batched_radiis_redistributed,
        "batched_depths_redistributed": batched_depths_redistributed,
        "gpui_to_gpuj_imgk_size": gpui_to_gpuj_imgk_size,
    }
...
def render_final(batched_screenspace_pkg, batched_strategies, tile_size=16):
    ...
    for cam_id in range(len(batched_screenspace_pkg["batched_rasterizers"])):
        ...
        #     print(f"[DEBUG] render_final: cam_id {cam_id}", flush=True)
        ...
        # DEBUG PRINT
        # if utils.GLOBAL_RANK == 0:
        #     print(f"[DEBUG] render_final: returned from render_gaussians", flush=True)
