...
class TileLossParams(ParamGroup):
    def __init__(self, parser):
        self.tile_loss_enable = False
        self.tile_loss_interval = 500
        self.tile_loss_start_iteration = 500
        self.tile_loss_weight = 0.1
        self.tile_loss_colmap = ""
        self.tile_loss_images_dir = ""
        self.tile_loss_image_idx = 0
        self.tile_loss_resolution_scale = 0.05
        self.tile_loss_output_dir = ""
        super().__init__(parser, "Tile Image Loss Parameters")

        alias_map = {
            "tile_loss_enable": "--tile-loss-enable",
            "tile_loss_interval": "--tile-loss-interval",
            "tile_loss_start_iteration": "--tile-loss-start-iteration",
            "tile_loss_weight": "--tile-loss-weight",
            "tile_loss_colmap": "--tile-loss-colmap",
            "tile_loss_images_dir": "--tile-loss-images-dir",
            "tile_loss_image_idx": "--tile-loss-image-idx",
            "tile_loss_resolution_scale": "--tile-loss-resolution-scale",
            "tile_loss_output_dir": "--tile-loss-output-dir",
        }

        for action in parser._actions:
            alias = alias_map.get(action.dest)
            if not alias:
                continue
            if alias in action.option_strings:
                continue
            action.option_strings.append(alias)
            parser._option_string_actions[alias] = action


class TileLoaderParams(ParamGroup):
    def __init__(self, parser):
        self.tile_scene_root = ""
        self.tile_plan_json = ""
        self.tile_plan_shuffle = False
        self.tile_batch_size = 1
        self.tile_steps_per_batch = 10
        self.tile_cycle_batches = True
        self.tile_write_back_root = ""
        super().__init__(parser, "Tile Loader Parameters")

        alias_map = {
            "tile_scene_root": "--tile-scene-root",
            "tile_plan_json": "--tile-plan-json",
            "tile_plan_shuffle": "--tile-plan-shuffle",
            "tile_batch_size": "--tile-batch-size",
            "tile_steps_per_batch": "--tile-steps-per-batch",
            "tile_cycle_batches": "--tile-cycle-batches",
            "tile_write_back_root": "--tile-write-back-root",
        }
...
class AdaptiveTileParams(ParamGroup):
    """Parameters for adaptive tile-based OOM-aware training."""
    def __init__(self, parser):
        # Tile bounding box: "x_min,y_min,z_min,x_max,y_max,z_max"
        self.tile_bbox = ""
        # Tile identifier
        self.tile_id = ""
