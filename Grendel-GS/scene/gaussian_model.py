...
class GaussianModel:
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            ...
            # Handle case where group name might be missing or different
            if "name" not in group:
                continue
                
            if group["name"] not in tensors_dict:
                # This can happen if we are densifying but some parameters (like f_rest) 
                # are not in the tensors_dict because they are empty or not being updated
                continue
