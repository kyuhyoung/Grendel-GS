#!/usr/bin/env bash
# 드론 세트 Let It Crash 런 (run_smoke_b200.sh 복제, 로그명만 SITE 별). 사용: SITE=paldalmun SRC=... OUT=... GPUS=... ITERS=... bash run_lic_drone.sh
# 확인 항목: ① [fused-ssim] enabled 로그 ② output/smoke_b200/adaptive_state.json 의 completed 타일 ③ merged/ 생성
set -uo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG="$HERE/run_lic_${SITE:-site}.log"; : > "$LOG"
...
export PET_MASTER_PORT="${MASTER_PORT:-29500}"   # 동시 실행 시 torchrun 포트 충돌 방지 (실측: EADDRINUSE)
export ADAPTIVE_DEBUG_SAVE="${ADAPTIVE_DEBUG_SAVE:-1}" SAVE_ITERS="${SAVE_ITERS:-4000,10000,20000,40000}"   # 디버깅용 중간 PLY (models/<tile>/point_cloud/iteration_N)
